...
| **Path C — hybrid storage: collapse high-cardinality patterns at fetch, keep named pages raw** | Pure raw URLs (original K2); pure pattern-only; exclude UUID routes entirely (Path B); accept truncation (Path D) | **K2 flipped after Phase 0 discovery (2026-05-21).** Real data showed web app has 4882 unique URLs/24h — 99% are partner-webview UUIDs saturating NerdGraph's 5000-facet cap. Pure-raw silently truncates; pure-pattern loses leadership-meaningful named pages. Path C: 4 collapse patterns + multiple exclude patterns + ~40 named routes raw. Validation: 19,438 vs 19,415 sample coverage (0.1% delta = excluded `/external/*` deprecated routes); 4899 → 30 facets per Q1 (99% headroom recovered). Pattern config lives in `route_patterns.csv` with `collapse_at_fetch` column driving both fetch logic AND dashboard rendering — single source of truth. |
