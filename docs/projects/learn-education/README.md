...
| FTI Users | `new_user_order` distinct `user_id` ∩ same-week cohort | conversion numerator |
| FTI users who watched | `new_user_order` ∩ `learn_video_viewed` (same user, any time before order) | causal-overlap proxy |
